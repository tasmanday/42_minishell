...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/31 20:06:47 by tday              #+#    #+#             */
/*   Updated: 2024/03/31 20:06:47 by tday             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	Summary
	prints the memory address of a pointer to the given file descriptor as a
	...
	[size_t] p: the memory address of the pointer.
	[int *] length: pointer to an integer that keeps count of how many
		characters have been printed.
	[int] first: a flag to indicate the first time the function has been called
...
void	ft_pointer_fd(int fd, size_t p, int *length, int first)
{
	...
	{
		ft_pointer_fd(fd, (p / 16), length, 0);
		ft_character_fd(fd, hexidecimal_base[p % 16], length);
	}
}
